#!/usr/bin/env python3
"""
Extract AirMuseum Scenario5 stereo+GT data for robotA into the flat-file
format this workspace's ORB-SLAM3 RGB-D loader expects (timestamp-named
left-camera PNGs + timestamps.txt + TUM-format ground truth). Mono-Depth
mode only -- no IMU extraction needed (that's only for Stereo-Inertial,
which is not used here).

Adapted from extract_airmuseum_data.py (the drone version) -- same ported
load_AirMuseum_data logic, parameterized for robotA instead.
"""
import numpy as np
...
from robotdataprocess import (
    ImageData, ImageDataOnDisk, OdometryData, CoordinateFrame, CameraData,
)
...
print(f"=== Writing left images to {left_out} (right discarded -- not needed for Mono-Depth) ===")
...
# ==================================== Config constants ====================================
print("\n" + "=" * 70)
print("CONFIG CONSTANTS (for RobotA_RGBD.yaml)")
print("=" * 70)
